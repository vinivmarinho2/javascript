...
let placar = document.querySelector ("div#placar")
botoes.forEach((botao) => { //"ForEach" percorre todos os itens da variável "botoes"
    ...
    botao.addEventListener("click", (elemento) => { // O "elemento" em parênteses representa o objeto do evento que ocorreu
        ...
        placar.innerHTML = `<p>Usuário: ${pontos_usuario} <br>Computador: ${pontos_computador} <br>Empates: ${empate}`
        // Comando de parada caso alguém chegue aos 5 pontos
        if (pontos_usuario === 5) {
            resultado.textContent = "Parabéns!! Você ganhou o jogo!"
            pontos_computador = 0 // Quando o jogo acaba, os pontos são reiniciados
            pontos_usuario = 0
            empate = 0
        } else if (pontos_computador === 5) {
...
// Só preciso salvar os pontos e parar o jogo assim que alguém faça 5 pontos
